...
// -- WinZip AE-x decryption -----------------------------------------
//
// `7z a -p<password> -mem=AES{128,192,256} archive.zip file.txt`
// produces an archive where the per-entry data is AES-CTR encrypted
// + HMAC-SHA1 authenticated.  Both fixtures below come from
// p7zip 23.01 with password "hunter2" and content
//   "winzip aes encrypted payload\n" (29 bytes).
// Layout: salt (16 B for AES-256 / 8 B for AES-128) + 2-byte
// verifier + ciphertext + 10-byte HMAC tag.  Compression method
// in the header is 99 (AE marker); the real method (0 = store)
// lives in the 0x9901 extra field.

const aex_expected_payload: BitArray = <<
  "winzip aes encrypted payload\n":utf8,
>>

const aex_password: String = "hunter2"

pub fn decode_system_winzip_aes256_archive_test() -> Nil {
  // `7z a -phunter2 -mem=AES256 aes256.zip secret.txt`
  let fixture = <<
    ,
  >>
  let assert Ok(decoded) =
    zip.decode_with_password(bytes: fixture, password: aex_password)
  let assert [restored] = archive.entries(decoded)
  entry.to_string(entry.path(restored))
  |> should.equal("secret.txt")
  entry.body(restored)
  |> should.equal(aex_expected_payload)
}

pub fn decode_system_winzip_aes128_archive_test() -> Nil {
  // `7z a -phunter2 -mem=AES128 aes128.zip secret.txt` — same content
  // but with a 16-byte AES key.  Salt drops from 16 B to 8 B; the
  // 0x9901 extra strength byte is 0x01 instead of 0x03.
  let fixture = <<
    ,
  >>
  let assert Ok(decoded) =
    zip.decode_with_password(bytes: fixture, password: aex_password)
...
pub fn decode_winzip_aes_rejects_wrong_password_test() -> Nil {
  // Same AES-256 fixture, but the password verifier doesn't match the
  // 2 bytes PBKDF2 derives from the wrong password — `resolve_aex_decryption`
  // short-circuits at the verifier check (BEFORE running AES + HMAC,
  // which is the AE-2 design: keep wrong-password rejection fast).
  let fixture = <<
    0x50, 0x4B, 0x03, 0x04, 0x33, 0x00, 0x01, 0x00, 0x63, 0x00, 0xBA, 0x6D, 0xBA,
    0x5C, 0x00, 0x00, 0x00, 0x00, 0x39, 0x00, 0x00, 0x00, 0x1D, 0x00, 0x00, 0x00,
    0x0A, 0x00, 0x0B, 0x00, 0x73, 0x65, 0x63, 0x72, 0x65, 0x74, 0x2E, 0x74, 0x78,
    ...
    0x01, 0x00, 0x67, 0x00, 0x00, 0x00, 0x6C, 0x00, 0x00, 0x00, 0x00, 0x00,
  >>
  case zip.decode_with_password(bytes: fixture, password: "definitely_wrong") {
    Error(error.ArchiveInvalid(_)) -> Nil
    _ -> should.fail()
  }
}
