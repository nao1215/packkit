...
pub fn archive_entry_by_path_returns_first_inserted_on_duplicate_test() -> Nil {
  // The lookup must match `entries(arc) |> list.find(...)` — that is,
  // the *first* entry inserted with the given path wins, not the
  // newest.  Pre-refactor the impl searched the internal reversed
  // list and returned the newest, breaking the contract the docstring
  // promised.
  let archive_value =
    tar.new()
    |> tar.add_file(path: "dup.txt", body: <<"first":utf8>>)
    |> tar.add_file(path: "dup.txt", body: <<"second":utf8>>)
...
pub fn archive_entry_by_path_matches_list_find_on_entries_test() -> Nil {
  // The function should be observationally equal to
  // `entries(arc) |> list.find(predicate)`.  Pin that as a law so the
  // implementation cannot drift back to searching the reversed
  // internal list.
  let archive_value =
    tar.new()
    |> tar.add_file(path: "a", body: <<"1":utf8>>)
    |> tar.add_file(path: "b", body: <<"2":utf8>>)
    |> tar.add_file(path: "a", body: <<"3":utf8>>)

  let via_find =
    archive.entry_by_path(archive_value, path: "a")
    |> result.map(entry.body)
  ...
  via_find
  |> should.equal(via_list_find)
}

pub fn gzip_with_name_round_trips_value_exactly_test() -> Nil {
  // `name(with_name(h, x))` must equal `Some(x)` for any `x` the
  // function accepts.  Earlier revisions stripped embedded NULs
  // silently, which violated the round-trip law.  The unchecked
  // variant now panics on NUL; ASCII / UTF-8 names that the function
  // accepts must round-trip unchanged.
  gzip.default_header()
  |> gzip.with_name(name: "report.csv")
  |> gzip.name
  |> should.equal(option.Some("report.csv"))

  gzip.default_header()
  |> gzip.with_name(name: "Naohiro_日本語_📦")
  |> gzip.name
  |> should.equal(option.Some("Naohiro_日本語_📦"))
}

pub fn gzip_with_comment_round_trips_value_exactly_test() -> Nil {
  gzip.default_header()
  |> gzip.with_comment(comment: "generated by packkit")
  |> gzip.comment
  |> should.equal(option.Some("generated by packkit"))
}

pub fn gzip_decode_payload_matches_decode_dot_payload_test() -> Nil {
  // Lawful pairing of the two-layer gzip decode API: the payload-only
  // variant returns exactly the `payload` slot of the rich variant.
  let plaintext = <<"hello, packkit!":utf8>>
  let assert Ok(stream_bytes) =
    packkit.compress(bytes: plaintext, with: codec.gzip())

  let via_payload = gzip.decode_payload(bytes: stream_bytes)
  let via_full =
    gzip.decode(bytes: stream_bytes)
    |> result.map(fn(d: gzip.Decoded) { d.payload })

  via_payload
  |> should.equal(via_full)

  via_payload
  |> should.equal(Ok(plaintext))
}
