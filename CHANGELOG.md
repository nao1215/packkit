...
- The doc comments of `limit.with_max_members` and `limit.with_max_entry_depth` now point to the clamp-to-1 contract of `limit.with_max_input_bytes`, like the other unchecked setters.
...
- **Fix (correctness)**: zstd 2-byte `Number_of_Sequences`
  ...
  more than 127 sequences read ~2× the actual count and then
  ran out of bitstream bits mid-decode (surfacing as
  "truncated zstd literal_length state-update bits").  The
  reference formula (see
  `doc/reference/zstd/lib/decompress/zstd_decompress_block.c`
  `ZSTD_decodeSeqHeaders`) is
  `((byte0 - 0x80) << 8) + byte1` — the 2-byte form FULLY
  OVERLAPS the 1-byte form and does not add a fresh 128 offset.
  Files generated by `zstd -3` of typical structured text
  (JSON-line, CSV, XML records) trigger this path; they all
  errored before and now decode again.
